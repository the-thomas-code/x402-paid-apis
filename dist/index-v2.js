...
const facilitatorClient = new server_2.HTTPFacilitatorClient({
    url: FACILITATOR_URL,
});
// Create x402 resource server with EVM scheme
const server = new express_2.x402ResourceServer(facilitatorClient);
server.register("eip155:*", new server_1.ExactEvmScheme());
// CORS setup
...
app.use((0, express_2.paymentMiddleware)(paymentConfig, server));
...
async function getBalance(address, token = "USDC", network = "base") {
...
async function getYieldRecommendations(amount, risk, duration = 30) {
    const strategies = {
        low: [
    ...
    const recommendationList = strategies[risk] || strategies.medium;
...
app.get("/api/balance", rateLimit(100, 60000), async (req, res) => {
    try {
        const { address, token = "USDC", network = "base" } = req.query;
        if (!address) {
            return res.status(400).json({ error: "Address parameter is required" });
        }
        const balance = await getBalance(address, token, network);
        res.json(balance);
    }
    catch (error) {
...
app.get("/api/yield", rateLimit(50, 60000), async (req, res) => {
    try {
        ...
        const recommendations = await getYieldRecommendations(parseFloat(amount), risk, parseInt(duration));
